...
//! `filesort` / `temporary_table` / `subquery_cache` wrappers), plus MariaDB's
//! `ANALYZE FORMAT=TEXT` indented output.
//!
//! Pure value -> [`ExplainNode`] transformations: no connection, no driver, no
...
pub fn parse_mysql_query_block(block: &serde_json::Value, counter: &mut u32) -> ExplainNode {
    let id = format!("node_{}", counter);
    *counter += 1;
...
pub fn has_analyze_data_recursive(node: &ExplainNode) -> bool {
...
pub fn parse_mysql_analyze_text(text: &str, counter: &mut u32) -> ExplainNode {
...
pub fn parse_analyze_actual(s: &str) -> (Option<f64>, Option<f64>, Option<u64>) {
